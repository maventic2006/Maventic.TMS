...
/**
 * ConsignorApprovalActionBar Component
 * Displays approval status and action buttons (Approve/Reject) for Consignor Admin users
 * Only visible to Product Owners who did NOT create the consignor
 */
const ConsignorApprovalActionBar = ({ userApprovalStatus, consignorId, onRefreshData }) => {
  ...
  if (!userApprovalStatus) {
    console.log("❌ ConsignorApprovalActionBar: No userApprovalStatus provided");
    return null;
  }

  console.log("🔍 ConsignorApprovalActionBar Debug:");
  console.log("  userApprovalStatus:", JSON.stringify(userApprovalStatus, null, 2));
  ...
  console.log("    currentApprovalStatus:", currentApprovalStatus);
  console.log("    pendingWithUserId:", pendingWithUserId);
  console.log("    user.user_id:", user?.user_id);

  // Check if current user is the assigned approver
  const isAssignedApprover = user?.user_id === pendingWithUserId;
  console.log("  isAssignedApprover:", isAssignedApprover);
  ...
  console.log("  showApprovalActions:", showApprovalActions);
  console.log("  Reason:", {
    isPending: currentApprovalStatus === "Pending for Approval",
    ...
    try {
      ...
      dispatch(
        addToast({
          ...
          message: "Consignor Admin user approved successfully",
        })
      );
      ...
      dispatch(
        addToast({
          ...
          message: error || "Failed to approve consignor admin user",
        })
      );
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      dispatch(
        addToast({
          type: TOAST_TYPES.SUCCESS,
          message: "Consignor Admin user rejected successfully",
        })
      );
      ...
      dispatch(
        addToast({
          type: TOAST_TYPES.ERROR,
          message: error || "Failed to reject consignor admin user",
        })
      );
    }
  };
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <p className="text-sm text-gray-600">
                  Please provide a reason for rejecting this consignor admin user. 
                  This will be sent to the creator for review.
                </p>
...
export default ConsignorApprovalActionBar;
