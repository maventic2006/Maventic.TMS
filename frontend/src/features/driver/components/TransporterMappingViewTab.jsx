...
const TransporterMappingViewTab = ({ driver }) => {
  const theme = getPageTheme('tab') || {};
  const transporterMappings = driver?.transporterMappings || [];
  ...
  if (transporterMappings.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400">
          No transporter mappings have been added yet.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {transporterMappings.map((item, index) => (
        <div
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                >
                  {item.transporterName || item.transporterId || 'N/A'}
                </h3>
                <p
                  className="text-sm"
                  style={{ color: safeTheme.colors.text.secondary }}
                >
                  ID: {item.transporterId || 'N/A'}
                </p>
              </div>
            </div>
