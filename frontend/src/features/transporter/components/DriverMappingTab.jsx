...
const DriverMappingTab = ({ transporterId }) => {
  const dispatch = useDispatch();
  const { driverMappings, mappingMasterData, isFetchingMappings } = useSelector(
  ...
  useEffect(() => {
    if (transporterId) {
      dispatch(fetchDriverMappings(transporterId));
  ...
  useEffect(() => {
    if (driverMappings) {
      ...
      const transformedMappings = driverMappings.map((mapping) => ({
        ...mapping,
  ...
  const columns = [
    {
      ...
      label: "Driver",
      type: "select",
      options: mappingMasterData?.drivers || [],
  ...
  const handleSaveAll = async () => {
    ...
    const invalidMappings = mappings.filter((m) => {
      return !m.driver_id || !m.valid_from;
    });
    ...
    if (invalidMappings.length > 0) {
      dispatch(
        addToast({
          type: TOAST_TYPES.ERROR,
          message:
            "Please fill all required fields (Driver and Valid From) for all mappings",
        })
      );
      return;
    }
    ...
    try {
      ...
      dispatch(
        addToast({
          type: TOAST_TYPES.SUCCESS,
          message: "Driver mappings saved successfully",
        })
      );
      dispatch(fetchDriverMappings(transporterId));
    } catch (error) {
  ...
  return (
    <div className="bg-[#F5F7FA]">
      <ThemeTable
        title="Transporter-Driver Mappings"
...
export default DriverMappingTab;
