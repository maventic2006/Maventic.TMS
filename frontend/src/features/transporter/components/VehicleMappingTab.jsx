...
const VehicleMappingTab = ({ transporterId }) => {
  ...
  useEffect(() => {
    if (transporterId) {
      dispatch(fetchVehicleMappings(transporterId));
    ...
    if (window.confirm("Are you sure you want to delete this mapping?")) {
      try {
        await dispatch(
          deleteVehicleMapping({
            transporterId,
            mappingId: mapping.tv_mapping_id,
          })
        ).unwrap();
        dispatch(
          addToast({
  ...
  const handleSaveAll = async () => {
    ...
    if (invalidMappings.length > 0) {
      dispatch(
        addToast({
          type: TOAST_TYPES.ERROR,
          message:
            "Please fill all required fields (Vehicle and Valid From) for all mappings",
        })
      );
      return;
    }
    ...
    try {
      ...
      dispatch(
        addToast({
          type: TOAST_TYPES.SUCCESS,
          message: "Vehicle mappings saved successfully",
        })
      );
      dispatch(fetchVehicleMappings(transporterId));
    } catch (error) {
  ...
  return (
    <div className="bg-[#F5F7FA]">
      <ThemeTable
        title="Transporter-Vehicle Mappings"
        titleIcon={Truck}
        data={mappings}
        columns={columns}
