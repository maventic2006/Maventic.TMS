...
const ConsignorMappingTab = ({ transporterId }) => {
  const dispatch = useDispatch();
  const { consignorMappings, mappingMasterData, isFetchingMappings } =
  ...
  useEffect(() => {
    if (consignorMappings) {
      // Transform backend data to match table format
      const transformedMappings = consignorMappings.map((mapping) => ({
        ...mapping,
        consignor_id: mapping.consignor_id || "",
  ...
  const columns = [
    {
      ...
      label: "Consignor",
      type: "select",
      options: mappingMasterData?.consignors || [],
    ...
    if (window.confirm("Are you sure you want to delete this mapping?")) {
      try {
        await dispatch(
          deleteConsignorMapping({
            transporterId,
            mappingId: mapping.tc_mapping_id,
          })
        ).unwrap();
        dispatch(
          addToast({
  ...
  const handleSaveAll = async () => {
    ...
    const invalidMappings = mappings.filter((m) => {
      return !m.consignor_id || !m.valid_from;
    });
    ...
    if (invalidMappings.length > 0) {
      dispatch(
        addToast({
          type: TOAST_TYPES.ERROR,
          message:
            "Please fill all required fields (Consignor and Valid From) for all mappings",
        })
      );
      return;
    }
    ...
    try {
      ...
      dispatch(
        addToast({
          type: TOAST_TYPES.SUCCESS,
          message: "Consignor mappings saved successfully",
        })
      );
      dispatch(fetchConsignorMappings(transporterId));
    } catch (error) {
  ...
  return (
    <div className="bg-[#F5F7FA]">
      <ThemeTable
        title="Transporter-Consignor Mappings"
        titleIcon={User}
        data={mappings}
        columns={columns}
...
export default ConsignorMappingTab;
